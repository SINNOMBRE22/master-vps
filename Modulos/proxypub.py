...
class ThreadingHTTPServer(ThreadingMixIn, HTTPServer):
    ...
    def handle_error(self, request, client_address):
        ...
        print >>sys.stderr, '-'*40
        print >>sys.stderr, 'Excepción ocurrida durante el procesamiento de solicitud desde', client_address
        traceback.print_exc()
        print >>sys.stderr, '-'*40
...
class SimpleHTTPProxyHandler(BaseHTTPRequestHandler):
    ...
    def decode_content_body(self, data, content_encoding):
        if content_encoding in ('gzip', 'x-gzip'):
            ...
            raise Exception("Codificación de contenido desconocida: %s" % content_encoding)
        return body
...
def test(HandlerClass=SimpleHTTPProxyHandler, ServerClass=ThreadingHTTPServer, protocol="HTTP/1.1"):
    ...
    print "Sirviendo HTTP en", sa[0], "puerto", sa[1], "..."
    httpd.serve_forever()
